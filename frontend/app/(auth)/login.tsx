    ...
    <SafeAreaView style={s.safe}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.kav}>
        ...
        <View style={s.logoWrap}>
          <View style={[s.logoIcon, glow(C.primary, 0.35)]}>
            <Ionicons name="hardware-chip" size={30} color={C.primary} />
          </View>
          <Text style={s.title}>DevOS</Text>
          <View style={s.pill}>
            <Text style={s.pillText}>AI Career Engine</Text>
          </View>
          <Text style={s.tagline}>Build faster. Land smarter.</Text>
        </View>

        {/* Form card */}
        <View style={s.card}>
          <Text style={s.cardLabel}>Welcome back</Text>
          ...
          <TouchableOpacity style={[s.btn, loading && s.btnDisabled, glow(C.primary, 0.3)]} onPress={handleLogin} disabled={loading} activeOpacity={0.85}>
            {loading
              ? <Text style={s.btnText}>Signing in…</Text>
              : <View style={s.btnInner}><Text style={s.btnText}>Sign In</Text><Ionicons name="arrow-forward" size={16} color="#fff" /></View>
            }
          </TouchableOpacity>
        </View>

        <TouchableOpacity onPress={() => router.push('/(auth)/signup')} style={s.footer} activeOpacity={0.7}>
          <Text style={s.footerText}>New here? <Text style={s.link}>Create an account →</Text></Text>
        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  logoIcon:   { width: 68, height: 68, borderRadius: 22, backgroundColor: C.primary + '18', borderWidth: 1, borderColor: C.primary + '40', alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
  title:      { fontSize: 48, fontWeight: '800', color: C.text, letterSpacing: -1.5 },
  pill:       { backgroundColor: C.primary + '20', borderWidth: 1, borderColor: C.primary + '40', borderRadius: 99, paddingHorizontal: 14, paddingVertical: 5, marginTop: 10 },
  pillText:   { color: C.primary, fontSize: 12, fontWeight: '700', letterSpacing: 0.5 },
